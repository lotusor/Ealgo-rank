...
| 管理后台前端 | ✅ | 仪表盘/学校/审批/爬虫/记录/成员/群发站内信 等页 + 角色化侧边菜单 + 深/浅色主题 |
...
**当前无进行中开发任务。** 最近一轮工作（2026-08-14）完成了权限体系收尾（#1~#5）与测试账户命令，全部已提交并通过验证（后端 57 tests OK，前端 typecheck 通过）。如需继续，从 §1.4 待办 / §1.5 技术债务切入。
...
manage.py check
manage.py test                 # 全量（当前 57 tests OK）
manage.py test apps.crawler    # 入库层测试（含作弊/学校绑定）
